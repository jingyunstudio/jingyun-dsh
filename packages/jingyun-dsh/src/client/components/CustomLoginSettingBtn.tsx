...
import { IconUserOutline16, IconSettingsOutline16 } from '@deepseek-ai/dsh-client-ui-primitives'
import {
  openLoginModal,
  openUpgradeModal,
  openRechargeModal,
...
export function CustomLoginSettingBtn(props: any) {
  ...
  React.useEffect(() => {
    ...
    const loadBrandingAndConfig = async () => {
      try {
        ...
        console.error('[UIBranding] Failed to load branding/public config:', err)
      }
    }
    ...
    const handleWindowBlur = () => {
      setTimeout(() => {
        if (
          ...
          (document.activeElement.tagName === 'IFRAME' || document.activeElement.tagName === 'iframe')
        ) {
          setShowMenu(false)
          setShowQrPopover(false)
        }
    ...
    if (typeof document !== 'undefined') {
      ...
      if (savedTheme) {
        ...
        brandingManager.fetch().then((data: any) => {
          const defaultTheme = data?.default_theme || data?.theme_mode || 'light'
  ...
  const toggleTheme = (mode: 'light' | 'dark', e?: React.MouseEvent) => {
    ...
    setThemeMode(mode)
    if (typeof document !== 'undefined') {
      try { localStorage.setItem('jy_theme_mode', mode) } catch (err) {}
      if (mode === 'dark') {
        document.documentElement.classList.add('dark')
        document.body.classList.add('dark')
        document.body.setAttribute('data-ds-dark-theme', 'true')
        document.documentElement.setAttribute('data-theme', 'dark')
        try { localStorage.setItem('dsw_theme', 'dark') } catch (err) {}
      } else {
        document.documentElement.classList.remove('dark')
        document.body.classList.remove('dark')
        document.body.removeAttribute('data-ds-dark-theme')
        document.documentElement.setAttribute('data-theme', 'light')
        try { localStorage.setItem('dsw_theme', 'light') } catch (err) {}
      }
  ...
  const handleOpenSettings = (e: React.MouseEvent) => {
    ...
    if (btnRef.current) {
      const dialogTrigger = btnRef.current.closest('button[aria-haspopup="dialog"]') as HTMLElement | null
      if (dialogTrigger) {
        dialogTrigger.click()
        return
      }
    ...
    if (typeof document !== 'undefined') {
      const dialogTrigger = document.querySelector('button[aria-haspopup="dialog"]') as HTMLElement | null
      if (dialogTrigger) {
        dialogTrigger.click()
        return
      }
  ...
  const displayName = user?.nickname || user?.username || user?.phone || '已登录'
  ...
  return (
    <div
      ...
      style={{ position: 'relative', width: '100%', padding: '6px 8px', boxSizing: 'border-box' }}
    >
      {/* 侧边栏底部按钮 */}
      <div
        ref={btnRef as any}
      ...
      >
        {isLogin && avatarUrl && !avatarErr ? (
          ...
          <IconUserOutline16 size={16} className="link-icon" style={{ flexShrink: 0 }} />
        )}

        {!isCollapsed && (
          <span style={{ fontWeight: isLogin ? 600 : 400, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', fontSize: isLogin ? '14px' : '13px' }}>
      ...
      {showQrPopover && wechatConfig?.wechat_qrcode_url && (
        <div
          ...
          style={{
            ...
            left: '8px',
            width: '180px',
            backgroundColor: 'var(--dsw-alias-bg-popover, var(--dsw-alias-bg-layer-2, #ffffff))',
        ...
        >
          ...
          <div style={{ display: 'flex', width: '100%', justifyContent: 'space-between', alignItems: 'center' }}>
            <span style={{ fontSize: '11px', fontWeight: 600, color: 'var(--dsw-alias-label-primary)' }}>手机扫码预览</span>
            <span
              style={{ fontSize: '13px', cursor: 'pointer', color: 'var(--dsw-alias-label-tertiary)', fontWeight: 'bold', padding: '0 4px' }}
          ...
          {/* 卡片底注 */}
          <span style={{ fontSize: '9px', color: 'var(--dsw-alias-label-tertiary)', textAlign: 'center' }}>
            使用微信扫码预览小程序
          </span>
        </div>
      )}
      ...
      {isLogin && showMenu && typeof document !== 'undefined' && createPortal(
        <div
          ref={menuRef}
        ...
        >
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '4px 8px 8px 8px' }}>
            ...
            <button
              ...
              style={{
                ...
                padding: '4px',
                borderRadius: '6px',
                color: copied ? '#16a34a' : 'var(--dsw-alias-label-tertiary, #71717a)',
                display: 'flex',
                alignItems: 'center',
                justifyContent: 'center',
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '2px' }}>
            ...
            <div 
              ...
              style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '8px 10px', borderRadius: '8px', fontSize: '13px', cursor: 'pointer' }}
            >
            ...
            >
              ...
              <span className="jy-menu-subtext" style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
            ...
            >
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M21 2l-2 2m-7.61 7.61a5.5 5.5 0 1 1-7.778 7.778 5.5 5.5 0 0 1 7.777-7.777zm0 0L15.5 7.5m0 0l3 3L22 7l-3-3m-3.5 3.5L19 4"></path></svg>
                <span>卡密激活</span>
              </div>
            </div>
          </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '2px' }}>
            {/* 设置 */}
            <div className="jy-menu-item" onClick={handleOpenSettings} style={{ display: 'flex', alignItems: 'center', padding: '8px 10px', borderRadius: '8px', fontSize: '13px', cursor: 'pointer', gap: '10px' }}>
            ...
            <div
              className="jy-menu-item"
              onClick={(e) => e.stopPropagation()}
              style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '6px 10px', borderRadius: '8px', fontSize: '13px' }}
            >
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="12" cy="12" r="10"></circle><path d="M12 2a7 7 0 1 0 10 10"></path></svg>
                <span>外观</span>
              </div>
              {/* 分段按钮开关 */}
            ...
            >
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M20 12v10H4V12"></path><path d="M2 7h20v5H2z"></path><path d="M12 22V7"></path><path d="M12 7H7.5a2.5 2.5 0 0 1 0-5C11 2 12 7 12 7z"></path><path d="M12 7h4.5a2.5 2.5 0 0 0 0-5C13 2 12 7 12 7z"></path></svg>
                <span>邀请有礼</span>
              </div>
...
function ConfigItemCard({ descriptor, onSaveConfig }: SingleCardProps) {
  ...
  const [dbDynamicValues, setDbDynamicValues] = React.useState<Record<string, string>>({})
  ...
  React.useEffect(() => {
    if (isBranding) {
      ...
      Object.keys(props).forEach(key => {
        initVals[key] = descriptor.value[key] !== undefined ? String(descriptor.value[key]) : (props[key].default || '')
      })
  ...
  const isDirty = isBranding
    ? (mode !== dbMode || apiUrl !== dbApiUrl || tenantHost !== dbTenantHost || appHost !== dbAppHost || customName !== dbCustomName || customLogo !== dbCustomLogo)
    : Object.keys(dynamicValues).some(key => dynamicValues[key] !== dbDynamicValues[key])

  const handleSave = async () => {
    setSaving(true)
    try {
      if (isBranding) {
        await onSaveConfig(descriptor.ns, { mode, apiUrl, tenantHost, appHost, customName, customLogo })
        setDbMode(mode)
        setDbApiUrl(apiUrl)
          ...
          {isBranding ? (
            <>
              ...
              <div className="jy-field">
                ...
                <div style={{ display: 'flex', gap: '24px', marginTop: '6px', padding: '2px 0' }}>
                  <label style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '13px', cursor: 'pointer', color: 'var(--dsw-alias-label-primary)' }}>
                    <input
                      type="radio"
                      name="jy-brand-mode"
                      value="cloud"
              ...
              {mode === 'cloud' ? (
                <>
                  <div className="jy-field" style={{ borderTop: '1px solid var(--dsw-alias-border-l2, #e4e4e7)' }}>
                    <div className="jy-field-head">
                      <label className="jy-field-label" htmlFor="jy-settings-api-url">服务地址</label>
                    </div>
                    <input
                    ...
                    />
                    <p className="jy-field-hint">SaaS 接口请求地址。留空默认需手动配置。</p>
                  </div>

                  <div className="jy-field" style={{ borderTop: '1px solid var(--dsw-alias-border-l2, #e4e4e7)' }}>
                    <div className="jy-field-head">
                      <label className="jy-field-label" htmlFor="jy-settings-tenant-host">租户标识</label>
                    </div>
                    <input
                    ...
                    />
                    <p className="jy-field-hint">租户唯一标识，用于鉴权与同步线上资源。留空默认需手动配置。</p>
                  </div>

                  <div className="jy-field" style={{ borderTop: '1px solid var(--dsw-alias-border-l2, #e4e4e7)' }}>
                    <div className="jy-field-head">
                      <label className="jy-field-label" htmlFor="jy-settings-app-host">终端域名</label>
                    </div>
                    <input
                    ...
                    />
                    <p className="jy-field-hint">Tauri 桌面端或独立终端的主页访问地址。留空默认需手动配置。</p>
                  </div>
                </>
              ) : (
                <>
                  <div className="jy-field" style={{ borderTop: '1px solid var(--dsw-alias-border-l2, #e4e4e7)' }}>
                    <div className="jy-field-head">
                      <label className="jy-field-label" htmlFor="jy-settings-custom-name">站点名称</label>
                    </div>
                    <input
                    ...
                    />
                    <p className="jy-field-hint">完全离线环境下系统展示的全局站点名称。默认值：为空自动展示原生标识</p>
                  </div>

                  <div className="jy-field" style={{ borderTop: '1px solid var(--dsw-alias-border-l2, #e4e4e7)' }}>
                    <div className="jy-field-head">
                      <label className="jy-field-label" htmlFor="jy-settings-custom-logo">站点 Logo</label>
                    </div>
                    <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                      ...
                      <button
                        ...
                        style={{ whiteSpace: 'nowrap', height: '34px', padding: '0 14px', boxSizing: 'border-box' }}
                        onClick={() => document.getElementById('jy-logo-file-picker')?.click()}
                      >
                        选择本地图片
                      </button>
                      <input
                    ...
                    <p className="jy-field-hint">系统左上角及网页 Favicon 小图标链接，支持 Base64 数据编码或公网 URL 地址。</p>
                  </div>
                </>
              )}
            </>
            ...
            Object.keys(descriptor.schema.properties || {}).map((fieldKey, idx) => {
              ...
              return (
                <div
                  key={fieldKey}
                  className="jy-field"
                  style={isFirst ? {} : { borderTop: '1px solid var(--dsw-alias-border-l2, #e4e4e7)' }}
                >
                  <div className="jy-field-head">
                    <label className="jy-field-label" htmlFor={`jy-settings-${descriptor.ns}-${fieldKey}`}>{fieldKey}</label>
                  </div>
                  <input
                  ...
                  />
                  {prop.description && <p className="jy-field-hint">{prop.description}</p>}
                </div>
              )
            })
          )}
...
export function AutoSchemaCards() {
  ...
  const fetchDescriptors = () => {
    fetch('/api/jingyun/branding/descriptors')
      ...
      .catch(err => console.error('[UIBranding] Failed to retrieve settings descriptors:', err))
  ...
  if (loading) {
    return <p style={{ fontSize: '13px', color: 'var(--dsw-alias-label-tertiary)' }}>正在获取配置项列表...</p>
  }

  if (descriptors.length === 0) {
    return <p style={{ fontSize: '13px', color: 'var(--dsw-alias-label-tertiary)' }}>暂无可配置扩展插件。</p>
  }
