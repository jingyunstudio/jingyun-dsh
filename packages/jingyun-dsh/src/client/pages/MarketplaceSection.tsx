    ...
    <div className="jy-marketplace-wrapper" style={{ width: '100%' }}>
      <div className="catalog">
        ...
        <label className="search">
          <svg width="14" height="14" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2.8" style={{ position: 'absolute', left: '12px', pointerEvents: 'none' }}>
        ...
        {isLoading ? (
          <div style={{ padding: '24px 0', textAlign: 'center', fontSize: '13px', color: 'var(--dsw-alias-label-tertiary)' }}>
            正在扫描已安装智能体列表...
          </div>
        ) : filteredItems.length === 0 ? (
          <div style={{ padding: '32px 0', textAlign: 'center', fontSize: '12px', color: 'var(--dsw-alias-label-tertiary)' }}>
            暂无已安装的智能体模块。
          </div>
        ) : (
          <ul className="cards">
            {filteredItems.map(item => {
              ...
              return (
                <li className="card" key={item.id} data-open={open ? 'true' : undefined}>
                  <button
                  ...
                  >
                    ...
                    <span className="cardTrailing">
                      ...
                      <svg className="chevron" width="12" height="12" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2.5" style={{ transition: 'transform 0.2s', transform: open ? 'rotate(180deg)' : 'rotate(0deg)' }}>
                  ...
                  {open && (
                    <div className="cardDetails">
                      <div className="descriptionText">{item.description || '暂无智能体详细描述'}</div>

                      
                      {Array.isArray(item.skills) && item.skills.length > 0 && (
                        <div style={{ marginTop: '10px', fontSize: '11px', color: 'var(--dsw-alias-label-secondary)', display: 'flex', flexWrap: 'wrap', gap: '4px', alignItems: 'center' }}>
                          <span>包含技能:</span>
                          {item.skills.map((s, sIdx) => (
                            <span key={sIdx} style={{ padding: '2px 7px', background: 'rgba(0,0,0,0.05)', borderRadius: '4px', fontSize: '11px' }}>
                              {s}
                            </span>
                          ))}
                        </div>
...
export const MarketplaceSkillsTab = () => {
  ...
  const activeLabel = menuItems.find(m => m.id === filterType)?.label || '全部'
  ...
  return (
    <div className="jy-marketplace-wrapper" style={{ width: '100%' }}>
      <div className="catalog">
        ...
        {/* Heading */}
        <div className="catalogHeading">
          <h3>{filterType === 'all' ? 'ClawHub 技能列表' : '已启用技能列表'}</h3>
          <span>{isLoading ? '加载中...' : filteredItems.length}</span>
        </div>
        ...
        {!isLoading && (
          <>
            <ul className="cards">
              {filteredItems.length === 0 ? (
                <div style={{ gridColumn: 'span 2', padding: '24px 0', textAlign: 'center', fontSize: '12px', color: 'var(--dsw-alias-label-tertiary)' }}>
                  {filterType === 'enabled' 
                    ? '暂无本地已下载启用的技能模块。' 
                    : '未检索到匹配该关键字的技能。'}
                </div>
                    ...
                    <li className="card" key={item.id} data-open={open ? 'true' : undefined}>
                      ...
                      >
                        ...
                        <span className="cardTrailing">
                          ...
                          <span className="configTag" data-enabled={isInstalled ? 'true' : 'false'}>
                            {configuration}
                          </span>
                          <svg className="chevron" width="12" height="12" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2.5" style={{ transition: 'transform 0.2s', transform: open ? 'rotate(180deg)' : 'rotate(0deg)' }}>
                            <path strokeLinecap="round" strokeLinejoin="round" d="M19 9l-7 7-7-7" />
                        ...
                        <div className="cardDetails">
                          ...
                          <div className="actionContainer">
                            {item.status === 'not_installed' && (
                              ...
                              >
                                {isCurrentInstalling ? '同步中...' : '同步到本地环境'}
                              </button>
                            )}
                            {isInstalled && <span className="infoText">✓ 该技能已加载到本地环境运行时中。</span>}
                          </div>
                        </div>
                      )}
                    </li>
              ...
              <div style={{ marginTop: '20px', marginBottom: '8px', textAlign: 'center' }}>
                <button
                  ...
                  style={{
                    ...
                    fontWeight: 500,
                    borderRadius: '8px',
                    border: '1px solid var(--dsw-alias-border-secondary, rgba(0,0,0,0.08))',
                ...
                >
                  ...
                  {!isLoadingMore && (
                    <svg width="12" height="12" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2.2" style={{ color: 'var(--dsw-alias-label-tertiary, #888)' }}>
                      <path strokeLinecap="round" strokeLinejoin="round" d="M19 9l-7 7-7-7" />
                    </svg>
                  )}
                </button>
...
export const MarketplaceCommunityPluginsTab = () => {
      ...
      if (res.ok) {
        ...
        if (json.success && Array.isArray(json.data)) {
          ...
          json.data.forEach((p: any) => {
            ...
            const isOfficial = p.is_builtin === true || 
                               p.source === 'builtin' || 
                               ...
                               (typeof p.id === 'string' && (p.id.startsWith('dsh-core-') || p.id.startsWith('builtin-')))
            ...
            if (!isOfficial && (p.source === 'community' || p.is_installed)) {
              mappedInstalled.push({
                ...
                owner: p.author || '社区插件',
                avatar: '',
                description: p.description_zh || p.description || '本地已安装社区插件',
                stars: 999,
                forks: 0,
  ...
  const loadCommunityNpmPlugins = async (targetPage = 1, searchQuery = '', isAppend = false) => {
    if (targetPage === 1) {
      setIsLoading(true)
    } else {
      setIsLoadingMore(true)
  ...
  const handleUninstallPlugin = async (targetPkg: string, pluginName: string) => {
    ...
    try {
      const res = await fetch('/api/jingyun/plugins/uninstall-community', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ npm: targetPkg, name: pluginName, repo: targetPkg })
      })
      const data = await res.json()
      if (data.success) {
        ...
        setInstalledItems(prev => prev.filter(p => 
  ...
  const handleInstallPlugin = async (repoFullName: string, pluginName: string) => {
    setInstallingRepo(repoFullName)
    try {
      const res = await fetch('/api/jingyun/plugins/install-community', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ repo: repoFullName, name: pluginName, npm: repoFullName })
      })
      const data = await res.json()
      if (data.success) {
        setInstalledPlugins(prev => {
  ...
  const activeFilterLabel = filterMenuItems.find(m => m.id === filterType)?.label || '全部'
  ...
  const filteredItems = sourceList.filter(item => {
    ...
    return (item.name && item.name.toLowerCase().includes(q)) ||
           ...
           (Array.isArray(item.topics) && item.topics.some((t: string) => t.toLowerCase().includes(q)))
  })
          ...
          <>
            <ul className="cards">
              {filteredItems.length === 0 ? (
                ...
                filteredItems.map(item => {
                  const open = expanded === item.id
                  const isInstalled = item.status === 'installed' || installedPlugins.has(item.name.toLowerCase()) || installedPlugins.has(item.fullName.toLowerCase())
                    ...
                    <li className="card" key={item.id} data-open={open ? 'true' : undefined}>
                      ...
                      >
                        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', overflow: 'hidden' }}>
                          {item.avatar ? (
                            ...
                            <div style={{ width: '20px', height: '20px', borderRadius: '50%', background: 'rgba(0,0,0,0.06)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '10px' }}>
                              🔌
                            </div>
                          )}
                          <strong className="cardTitle" style={{ maxWidth: '160px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                            {item.name}
                          </strong>
                        </div>

                        <span className="cardTrailing">
                          <span style={{ fontSize: '11px', color: 'var(--dsw-alias-label-tertiary)', display: 'flex', alignItems: 'center', gap: '2px', marginRight: '4px' }}>
                            ⭐ {item.stars}
                          </span>
                          <span className="statusDot" data-phase={isInstalled ? 'active' : 'unobserved'} />
                          <span className="configTag" data-enabled={isInstalled ? 'true' : 'false'}>
                      ...
                      {open && (
                        <div className="cardDetails">
                          ...
                          <div style={{ marginTop: '8px', fontSize: '11px', color: 'var(--dsw-alias-label-tertiary)', display: 'flex', flexWrap: 'wrap', gap: '10px', alignItems: 'center' }}>
                            <span>作者: <strong>{item.owner}</strong></span>
                            {item.updatedAt && <span>更新时间: {item.updatedAt}</span>}
                          ...
                          {Array.isArray(item.topics) && item.topics.length > 0 && (
                            <div style={{ marginTop: '8px', display: 'flex', flexWrap: 'wrap', gap: '4px' }}>
                              {item.topics.slice(0, 6).map((t, idx) => (
                                <span key={idx} style={{ padding: '1px 6px', background: 'rgba(0,0,0,0.04)', borderRadius: '3px', fontSize: '10px', color: 'var(--dsw-alias-label-secondary)' }}>
                                  #{t}
                                </span>
                              ))}
                            </div>
                          ...
                          <div className="actionContainer" style={{ marginTop: '12px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                            {item.url ? (
                              <a
                                href={item.url}
                                target="_blank"
                                rel="noopener noreferrer"
                                style={{ fontSize: '11px', color: 'var(--dsw-alias-color-primary, #3b82f6)', textDecoration: 'none', display: 'flex', alignItems: 'center', gap: '4px' }}
                            ...
                            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                              {!isInstalled ? (
                                <button
                                  type="button"
                                  disabled={isCurrentInstalling}
                                  onClick={() => handleInstallPlugin(item.fullName || item.name, item.name)}
                                  className="actionBtn primary"
                                >
                                  {isCurrentInstalling ? '正在安装到底座...' : '一键安装到底座'}
                                </button>
                              ) : (
                                <button
                                  type="button"
                                  disabled={Boolean(uninstallingRepo && (uninstallingRepo === item.fullName || uninstallingRepo === item.npm))}
                                  onClick={() => handleUninstallPlugin(item.npm || item.fullName || item.name, item.name)}
                                  className="actionBtn secondary"
                                  style={{ color: '#ef4444', borderColor: 'rgba(239, 68, 68, 0.3)', padding: '4px 12px', fontSize: '12px' }}
                                >
                                  {uninstallingRepo && (uninstallingRepo === item.fullName || uninstallingRepo === item.npm) ? '正在卸载...' : '卸载'}
                                </button>
                              )}
                            </div>
                          </div>
