...
export class WecomConnectorService {
    ...
    try {
      ...
      if (
        ...
        (!this.config.gatewayUrl ||
          this.config.gatewayUrl.includes('wework_admin'))
      ) {
  ...
  public async connect(overrideConfig?: WecomConfig): Promise<void> {
    ...
    console.log(
      `[WecomConnector] Connecting to official gateway ${wsUrl} for bot: ${this.maskBotId(this.config.botId)}`
    );
    ...
    try {
      ...
      this.ws.on('open', () => {
        console.log(
          '[WecomConnector] WebSocket TCP connected, sending aibot_subscribe...'
        );
      ...
      this.ws.on('close', (code: number, reason: Buffer) => {
        ...
        console.log(
          `[WecomConnector] Connection closed. code=${code}, reason=${reasonStr}`
        );
        this.stopHeartbeat();
  ...
  private handleMessage(data: RawData): void {
    try {
      ...
      if (reqId.startsWith('aibot_subscribe')) {
        if (frame.errcode === 0) {
          console.log(
            '[WecomConnector] ✅ Official authentication successful! Bot is online.'
          );
          ...
          console.error(
            `[WecomConnector] ❌ Authentication failed: ${frame.errmsg} (code: ${frame.errcode})`
          );
          this.status = 'error';
    ...
    this.heartbeatTimer = setInterval(() => {
      if (this.ws && this.ws.readyState === WebSocket.OPEN) {
        if (this.missedPongCount >= this.maxMissedPong) {
          console.warn(
            '[WecomConnector] Heartbeat ack timeout, terminating connection to reconnect.'
          );
          this.ws.terminate();
          return;
        }
