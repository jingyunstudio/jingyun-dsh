...
; Set the compression algorithm. Since vendor resources (Node/Git/Python/Deps) are already pre-compressed .zip files,
; disabling NSIS solid LZMA compression prevents CPU bottleneck and reduces packaging time from 10+ mins to 20 seconds.
SetCompress off
...
; Installer header image (disabled for clean text-only header)
; !if "${HEADERIMAGE}" != ""
...
Function CustomGUIInit
  ; Hide top-right logo icon in header (Control ID 1039 in NSIS Modern UI)
...
!define MUI_PAGE_CUSTOMFUNCTION_PRE SkipIfPassive
!define MUI_PAGE_CUSTOMFUNCTION_SHOW HideHeaderLogo
