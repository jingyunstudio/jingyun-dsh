...
const DEFAULT_LAST_CHECK_FILE = path.join(os.tmpdir(), 'ima_last_update_check');
...
async function checkSkillUpdateIfNeeded(requestOptions) {
  ...
  const latestVersion =
    (updateResp.data && updateResp.data.latest_version) || '';
...
async function imaApi(apiPath, body, options = {}) {
  const forceCheck =
    ...
    process.env.IMA_FORCE_UPDATE_CHECK === '1';
  const baseUrl =
    options.baseUrl || process.env.IMA_BASE_URL || DEFAULT_BASE_URL;
  const lastCheckFile =
    options.lastCheckFile ||
    process.env.IMA_LAST_CHECK_FILE ||
...
async function main() {
  const [, , apiPath, rawBody = '{}', rawOptions = '{}', ...rest] =
    process.argv;
  ...
  try {
    ...
    const code =
      err && typeof err.code === 'number' ? err.code : ERR_PROGRAMMATIC;
