...
export function NavigationRows({ wide = true }: NavigationRowsProps) {
  ...
  const [topPortalTarget, setTopPortalTarget] = React.useState<HTMLElement | null>(null)

  const handleNavClick = (e: React.MouseEvent, targetHash: string, requiresConfig: boolean) => {
    e.stopPropagation()
    if (!requiresConfig) {
  ...
  React.useEffect(() => {
    ...
    const newChatBtn = document.querySelector('button[class*="newSession"]') || 
    ...
    if (newChatBtn) {
      const labelSpan = newChatBtn.querySelector('[class*="newSessionLabel"]') || newChatBtn.querySelector('span')
      ...
      const btnObserver = new MutationObserver(() => {
        const span = newChatBtn.querySelector('[class*="newSessionLabel"]') || newChatBtn.querySelector('span')
        if (span && span.textContent !== '新建任务' && span.textContent !== '') {
          span.textContent = '新建任务'
        }
      })
      btnObserver.observe(newChatBtn, { childList: true, subtree: true, characterData: true })

      if (newChatBtn.parentElement) {
        let portalDiv = newChatBtn.parentElement.querySelector('#jy-nav-top-portal') as HTMLElement | null
        if (!portalDiv) {
  ...
  const navContent = (
    <div className="jy-sidebar-custom-links" style={{ width: '100%', display: 'flex', flexDirection: 'column', gap: '2px' }}>
      {/* 1. 应用市场 */}
      <button
        type="button"
        ...
        onClick={(e) => handleNavClick(e, '#/jingyun/more?path=%2Fzh%2Fmarketplace', true)}
      >
      ...
      <button
        ...
        onClick={(e) => handleNavClick(e, '#/jingyun/more?path=%2Fzh%2Fmy-assets', true)}
      >
      ...
      >
        <IconProjectAddOutline16 size={16} className="link-icon" style={{ marginRight: isCollapsed ? '0' : '12px' }} />
        {!isCollapsed && <span>更多</span>}
        {!isCollapsed && (
          <span style={{ fontSize: '11px', color: 'var(--dsw-alias-label-tertiary, #9e9e9e)', marginLeft: 'auto', paddingRight: '4px' }}>
            线上资源
          </span>
        )}
      </button>
...
function OnlineWorkspaceOverlay() {
  ...
  React.useEffect(() => {
    ...
    const handleGlobalClick = (e: MouseEvent) => {
      const hash = window.location.hash || ''
      const isOverlay = hash.startsWith('#/jingyun/more') || hash === '#/jingyun/connectors' || hash === '#/jingyun/automation'
      if (!isOverlay) return
      const target = e.target as HTMLElement
      if (!target) return

      // 过滤折叠/展开侧边栏按钮，避免误触发退出
      const isToggleBtn = target.closest('button[class*="toggle"]') ||
      ...
      const isMoreControl = target.closest('.jy-sidebar-custom-links') ||
        target.closest('.jy-sidebar-link-more') || 
        target.closest('.jy-more-dropdown') || 
        target.closest('.jy-workspace-overlay')

      const isSwitchViewInteraction = target.closest('button[class*="newSession"]') ||
  ...
  const isOverlayActive = currentHash.startsWith('#/jingyun/more') || isConnectorsRoute || isAutomationRoute
  ...
  React.useEffect(() => {
    ...
    const mainEl = sidebar && sidebar.parentElement 
      ? (Array.from(sidebar.parentElement.children).find(el => el !== sidebar) as HTMLElement)
      : null
  ...
  const [currentTheme, setCurrentTheme] = React.useState<string>(() => {
    if (typeof document !== 'undefined') {
      ...
      return (typeof localStorage !== 'undefined' && localStorage.getItem('jy_theme_mode')) || 'light'
    }
    return 'light'
  })
  ...
  React.useEffect(() => {
    const handleThemeChange = (e: any) => {
      const newTheme = e.detail || (typeof localStorage !== 'undefined' ? localStorage.getItem('jy_theme_mode') : 'light')
      if (newTheme) {
        setCurrentTheme(newTheme)
        // 给当前的 iframe 实例即时发送 postMessage
        const iframeEl = document.querySelector('.jy-workspace-overlay iframe') as HTMLIFrameElement
        if (iframeEl && iframeEl.contentWindow) {
          try {
            iframeEl.contentWindow.postMessage({ type: 'JY_THEME_CHANGE', theme: newTheme }, '*')
          } catch (err) {}
        }
      }
    }
    ...
    return () => window.removeEventListener('jy_theme_change', handleThemeChange as any)
  }, [])
      ...
      if (type === 'JY_CREATE_AGENT_ACTION') {
        ...
        setTimeout(() => {
          ...
          const newChatBtn = allButtons.find(b => {
            ...
            return t.includes('新建') || t.includes('New') || b.className.includes('newSession')
          }) as HTMLElement
          ...
          setTimeout(() => {
            if (prompt && typeof window !== 'undefined') {
              ...
              if (textareas.length > 0) {
                const targetInput = (document.activeElement?.tagName === 'TEXTAREA' 
  ...
  try {
    ...
    const cachedToken = typeof localStorage !== 'undefined' ? localStorage.getItem('jy_online_token') : null
    if (cachedToken) {
