...
function RealArtifactRenderer({ file, showCode }: { file: ArtifactTabItem; showCode: boolean }) {
  ...
  if (file.type === 'html') {
    if (showCode) {
      return (
        <div style={{ padding: '24px', overflowY: 'auto', height: '100%', backgroundColor: 'var(--dsw-alias-bg-layer-3, #ffffff)' }}>
          <pre style={{ whiteSpace: 'pre-wrap', wordBreak: 'break-all', fontFamily: 'SFMono-Regular, Consolas, Monaco, monospace', fontSize: '13.5px', lineHeight: '1.65', margin: 0, color: 'var(--dsw-alias-label-primary, #09090b)' }}>
            {content || `<!-- 真实产物: ${file.name} 正在读取或暂无文本内容 -->`}
          </pre>
        </div>
      )
    }
    ...
    return (
      <iframe
        ...
        srcDoc={content || `<!DOCTYPE html><html><body style="font-family:sans-serif;padding:30px;color:#666;text-align:center;"><h3>${file.name}</h3><p>正在读取物理文件真实内容...</p></body></html>`}
        title={file.name}
        style={{ width: '100%', height: '100%', flex: 1, border: 'none', backgroundColor: '#ffffff' }}
    ...
    return (
      <div style={{ padding: '24px', overflowY: 'auto', height: '100%', backgroundColor: 'var(--dsw-alias-bg-layer-3, #ffffff)' }}>
        {content ? (
          ...
          <div style={{ color: 'var(--dsw-alias-label-secondary, #52525b)', fontSize: '13px' }}>
            <h3 style={{ fontSize: '16px', fontWeight: 600, margin: '0 0 8px 0', color: 'var(--dsw-alias-label-primary, #09090b)' }}>{file.name}</h3>
...
export function ArtifactInspectorPanel({ ctx }: { ctx?: any }) {
  ...
  const [artifactsExpanded, setArtifactsExpanded] = React.useState<boolean>(true)
  const [showCodePreview, setShowCodePreview] = React.useState<boolean>(false)

  // 计算左右 50% / 50% 完美均分宽度
  const getDefaultSplitWidth = () => {
    if (typeof window !== 'undefined') {
      const sidebarEl = document.querySelector('[class*="sidebar"], [class*="SideBar"]') as HTMLElement | null
  ...
  const [panelWidth, setPanelWidth] = React.useState<number>(() => {
    if (typeof window !== 'undefined') {
      ...
      if (saved) {
        ...
        if (!isNaN(parsed) && parsed >= 320 && parsed <= window.innerWidth - 320) {
          return parsed
        }
      }
  ...
  const handleResizeStart = (e: React.MouseEvent) => {
    ...
    const handleMouseMove = (moveEvent: MouseEvent) => {
      ...
      const maxAllowed = Math.max(minArtifactWidth, window.innerWidth - sidebarWidth - minChatWidth)
      const newWidth = Math.max(minArtifactWidth, Math.min(maxAllowed, startWidth + deltaX))
      setPanelWidth(newWidth)
      document.documentElement.style.setProperty('--jy-artifact-panel-width', `${newWidth}px`)
    }
  ...
  React.useEffect(() => {
    if (open && !fullscreen) {
      document.documentElement.style.setProperty('--jy-artifact-panel-width', `${panelWidth}px`)
    ...
    const handleWindowResize = () => {
      ...
      setPanelWidth((prev) => {
        ...
        if (prev > maxAllowed) {
          ...
          document.documentElement.style.setProperty('--jy-artifact-panel-width', `${clamped}px`)
          return clamped
        }
        return prev
      })
  ...
  const handleToggleFullscreen = () => {
    if (fullscreen) {
      ...
      const validWidth = panelWidth > maxAllowed || panelWidth >= window.innerWidth - 300 ? getDefaultSplitWidth() : panelWidth
      setPanelWidth(validWidth)
      document.documentElement.style.setProperty('--jy-artifact-panel-width', `${validWidth}px`)
      setFullscreen(false)
    } else {
      setFullscreen(true)
    }
  ...
  const activeTabItem = openTabs.find((t) => t.id === activeTabId) || openTabs[0]
  ...
  React.useEffect(() => {
    ...
    const fetchPhysicalArtifact = async (fileName: string) => {
      try {
        ...
        const res = await fetch(`/api/jingyun/artifact/read?file=${encodeURIComponent(fileName)}&sessionId=${encodeURIComponent(sessionId)}`)
    ...
    const openArtifactFile = async (fileName: string, rawContent?: string, filePath?: string, explicitType?: ArtifactTabItem['type']) => {
      ...
      if (!explicitType) {
        ...
        else if (/\.(png|jpg|jpeg|gif|webp|svg)$/i.test(fileName)) fileType = 'image'
      }
    ...
    const handleSpillArtifact = (e: CustomEvent<SaveTextSpill | any>) => {
      ...
      const fileName = detail.suggestedName || detail.name || detail.id || '产物文件'
    ...
    const handleGlobalClick = (e: MouseEvent) => {
      ...
      const title = target.getAttribute('title') || target.closest('button, code, a')?.getAttribute('title') || ''
      const rawText = title || text
      const match = rawText.match(/[a-zA-Z0-9_\u4e00-\u9fa5\-]+\.(html|md|png|jpg|jpeg|json|js|ts|css|py|vue|tsx|jsx)/i)
      const isProducedContext = Boolean(
        target.closest('[data-produced-files-row], [class*="turnTail"], [class*="produced"], [class*="deliverable"], [class*="Write"], [class*="artifact"]')
      ) || 
      text.includes('产物') || 
      ...
      if (sidebarOrNavClick && !isProducedContext) {
        ...
        const isSwitchPage =
          ...
          Boolean(target.closest('[class*="session"], [class*="Session"], [class*="history"], [class*="workspace-item"], [class*="navItem"]'))
      ...
      if (match && isProducedContext) {
        ...
        fileName = fileName.replace(/^Write\s*/i, '').replace(/^Edit\s*/i, '').replace(/^产物\s*/, '').trim()

        if (fileName && /\.(html|md|png|jpg|jpeg|json|js|ts|css|py|vue|tsx|jsx)/i.test(fileName)) {
          e.stopPropagation()
          e.preventDefault()

          // 尝试从节点 dataset 获取已绑定数据，若无则触发 API 异步读盘
          const containerNode = target.closest('[data-artifact-content], [data-artifact-path]') as HTMLElement | null
    ...
    window.addEventListener('jy_open_artifact_inspector' as any, handleSpillArtifact)
    window.addEventListener('dsh_spill_artifact' as any, handleSpillArtifact)
    window.addEventListener('jy_toggle_artifact_inspector' as any, handleToggle)
    window.addEventListener('jy_close_artifact_inspector' as any, handleAutoClose)
    window.addEventListener('popstate', handleAutoClose)
    document.addEventListener('click', handleGlobalClick, true)

    return () => {
      window.removeEventListener('jy_open_artifact_inspector' as any, handleSpillArtifact)
      window.removeEventListener('dsh_spill_artifact' as any, handleSpillArtifact)
      window.removeEventListener('jy_toggle_artifact_inspector' as any, handleToggle)
      window.removeEventListener('jy_close_artifact_inspector' as any, handleAutoClose)
  ...
  const scanDOMArtifacts = React.useCallback(() => {
    ...
    const producedContainers = document.querySelectorAll('[data-produced-files-row]')
    ...
    producedContainers.forEach((container) => {
      ...
      buttons.forEach((btn) => {
        ...
        if (textName && /\.(html|md|png|jpg|jpeg|json|js|ts|css|py|vue|tsx|jsx)/i.test(textName)) {
          ...
          if (!addedIds.has(fileId)) {
            ...
            else if (/\.(png|jpg|jpeg|gif|webp|svg)$/i.test(textName)) fileType = 'image'
    ...
    try {
      ...
      const handleSessionsUpdate = () => {
        try {
          ...
          console.warn('[ArtifactPanel] Failed to update session ID state:', err)
        }
      }
  ...
  React.useEffect(() => {
    ...
    if (currentSessionId && lastSessionIdRef.current && currentSessionId !== lastSessionIdRef.current) {
      setOpen(false)
      setOpenTabs([])
      setActiveTabId('')
      setArtifactList([])
  ...
  const handleRefresh = async () => {
    ...
    try {
      const res = await fetch(`/api/jingyun/artifact/read?file=${encodeURIComponent(activeTabItem.path || activeTabItem.name)}`)
      if (res.ok) {
        const data = await res.json()
        if (data.success && data.content) {
          setOpenTabs((prev) =>
            prev.map((t) => (t.id === activeTabItem.id ? { ...t, content: data.content, path: data.path || t.path } : t))
          )
          setArtifactList((prev) =>
            prev.map((t) => (t.id === activeTabItem.id ? { ...t, content: data.content, path: data.path || t.path } : t))
          )
  ...
  const handleRevealFolder = async () => {
    ...
    try {
      await fetch(`/api/jingyun/artifact/reveal?path=${encodeURIComponent(activeTabItem.path)}`)
    } catch (e) {
  ...
  const handleOpenExternal = async () => {
    ...
    try {
      ...
      await fetch(`/api/jingyun/artifact/open-external?path=${encodeURIComponent(activeTabItem.path)}`)
    } catch (e) {
  ...
  const renderMinimalFileIcon = (type: ArtifactTabItem['type']) => {
    switch (type) {
      case 'html':
        return (
          <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" style={{ flexShrink: 0, opacity: 0.75 }}>
  ...
  return (
    ...
    >
      ...
      {!fullscreen && (
        <div
          ...
          onDoubleClick={() => {
            ...
            document.documentElement.style.setProperty('--jy-artifact-panel-width', `${defaultWidth}px`)
            localStorage.setItem('jy_artifact_panel_width', String(defaultWidth))
          }}
        />
      )}
          ...
          {(popoverOpen || pinned) && (
            ...
            >
              <div className="jy-minimal-drawer-header">
                <span style={{ fontSize: '13px', fontWeight: 600, color: 'var(--dsw-alias-label-primary, #09090b)' }}>
                  产物资源库
                </span>
              </div>
              ...
              <div className="jy-minimal-drawer-body">
                ...
                >
                  ...
                  <svg
                    ...
                    style={{
                      transform: artifactsExpanded ? 'rotate(0deg)' : 'rotate(-90deg)',
                ...
                {artifactsExpanded && (
                  <div className="jy-minimal-artifact-list">
                    {artifactList.length === 0 ? (
                      <div style={{ padding: '8px 10px', fontSize: '12px', color: '#a1a1aa' }}>暂无生成产物</div>
                    ) : (
                      artifactList.map((item) => (
                        ...
                        >
                          ...
                          <span className="jy-minimal-file-name">{item.name}</span>
                        </div>
                      ))
                    )}
                  </div>
      ...
      <div className="jy-artifact-workspace-body">
        {/* 右侧内容主渲染区 */}
        <div className="jy-artifact-main-viewer" style={{ flex: 1, display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden' }}>
          {activeTabItem ? (
            <div className="jy-html-browser-container" style={{ flex: 1, display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden' }}>
              {/* 统一的顶部地址栏 & 操作按钮组 */}
              <div className="jy-browser-address-bar" style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '6px 12px', borderBottom: '1px solid var(--dsw-alias-border-l2, rgba(0, 0, 0, 0.08))', backgroundColor: 'var(--dsw-alias-bg-layer-3, #ffffff)', flexShrink: 0 }}>
                {/* 左侧前进/后退 */}
                ...
                {/* 中间地址胶囊栏 */}
                <div className="jy-browser-url-input" style={{ flex: 1, display: 'flex', alignItems: 'center', gap: '6px', background: 'var(--dsw-alias-bg-layer-2, #f4f4f5)', borderRadius: '6px', padding: '4px 10px', fontSize: '12px', color: 'var(--dsw-alias-label-secondary, #52525b)', overflow: 'hidden' }}>
                  <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" style={{ flexShrink: 0, opacity: 0.7 }}>
                    <circle cx="12" cy="12" r="10" />
                    <line x1="2" y1="12" x2="22" y2="12" />
                    <path d="M12 2a15.3 15.3 0 0 1 4 10 15.3 15.3 0 0 1-4 10 15.3 15.3 0 0 1-4-10 15.3 15.3 0 0 1 4-10z" />
                  </svg>
                  <span style={{ textOverflow: 'ellipsis', overflow: 'hidden', whiteSpace: 'nowrap', flex: 1, userSelect: 'all', fontFamily: 'SFMono-Regular, Consolas, monospace' }}>
                    {activeTabItem.path?.startsWith('file://') ? activeTabItem.path : (activeTabItem.path ? `file:///${activeTabItem.path.replace(/\\/g, '/')}` : activeTabItem.name)}
                  </span>
                </div>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
                  ...
                  {activeTabItem.type === 'html' && (
                    <button
                      ...
                      title={showCodePreview ? '切换到网页渲染' : '查看HTML源码'}
                      ...
                      style={{
                        ...
                        borderRadius: '4px',
                        border: 'none',
                        background: showCodePreview ? 'rgba(0, 0, 0, 0.08)' : 'transparent',
                        cursor: 'pointer',
                        color: showCodePreview ? 'var(--dsw-alias-label-primary, #09090b)' : 'var(--dsw-alias-label-secondary, #71717a)'
                      }}
                    >
                  ...
                  <button
                    ...
                    style={{ width: '28px', height: '28px', display: 'flex', alignItems: 'center', justifyContent: 'center', borderRadius: '4px', border: 'none', background: 'transparent', cursor: 'pointer', color: 'var(--dsw-alias-label-secondary, #71717a)' }}
                  >
                    <svg className={isRefreshing ? 'jy-spin' : ''} width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
              ...
              <div style={{ flex: 1, height: 'calc(100% - 42px)', overflow: 'hidden' }}>
                <RealArtifactRenderer file={activeTabItem} showCode={showCodePreview && activeTabItem.type === 'html'} />
              </div>
            </div>
          ) : (
            <div style={{ padding: '32px', color: '#a1a1aa', fontSize: '13px', textAlign: 'center' }}>
              暂无打开的产物
            </div>
          )}
        </div>
