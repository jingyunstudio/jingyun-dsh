...
export const Config: Schema<Config> = Schema.object({
  mode: Schema.union([Schema.const('cloud'), Schema.const('local')])
    .default('cloud')
    .description(
  ...
  customName: Schema.string()
    .default('')
    .description('离线自定义模式下系统展示的站点名称。留空默认展示原生标识。'),
  customLogo: Schema.string()
    .default('')
    .description(
      '离线自定义模式下系统展示的 Logo 图片链接或 Base64 编码。留空默认展示原生标识。'
    ),
});
