...
import { getSessionAgentsConfig, saveSessionAgentsConfig } from '../agent/manager'
    ...
    handler: async (req, res) => {
      ...
      req.on('end', async () => {
        try {
          ...
          console.log(`[UIBranding] Session ${sessionId} agent activated to: ${agentId}`)

          sendJson(res, { success: true, message: 'Agent activated successfully.' })
        } catch (err: any) {
