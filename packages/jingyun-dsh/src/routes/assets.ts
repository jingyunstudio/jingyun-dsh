          ...
          if (fs.statSync(p).isDirectory()) {
            ...
            if (!desc) {
              if (name === 'skill-creator')
                ...
                desc = '图片生成与创意作图工具';
              else if (name === 'word-docx-1')
                desc = 'Word 文档读写与样式排版工具';
    ...
    handler: async (req, res) => {
      ...
      req.on('end', async () => {
        try {
              ...
              JSON.stringify(
                {
                  ...
                  description:
                    payload.description_zh || payload.description || '',
                  source: 'marketplace',
          ...
          sendJson(res, {
            success: true,
            message: `Asset "${slug}" installed successfully.`,
          });
        } catch (err: any) {
    ...
    handler: async (req, res) => {
      ...
      req.on('end', async () => {
        try {
          ...
          sendJson(res, {
            success: true,
            message: `Asset "${cleanSlug}" uninstalled.`,
          });
        } catch (err: any) {
          sendError(res, err.message);
        }
      ...
      req.on('end', async () => {
        try {
          ...
          const hasManifest = fs.existsSync(
            path.join(extractTempDir, 'manifest.json')
          );
          const hasSkillsets = fs.existsSync(
            path.join(extractTempDir, 'skillsets')
          );
            ...
            if (fs.existsSync(bundledDir)) {
              ...
              for (const zf of zipFiles) {
                ...
                if (fs.existsSync(sDest))
                  fs.rmSync(sDest, { recursive: true, force: true });
