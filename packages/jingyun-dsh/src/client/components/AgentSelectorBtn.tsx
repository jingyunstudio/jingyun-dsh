...
export const AgentSelectorBtn = (props: any) => {
  ...
  const fetchAgents = React.useCallback(
    async (sessId?: string) => {
      ...
      const param =
        targetSessId && targetSessId !== 'default'
          ? `?sessionId=${encodeURIComponent(targetSessId)}`
          : '';
      const endpoints = [
        ...
        try {
          ...
          if (res.ok) {
            ...
            if (rawAgents.length > 0 || json.data?.skillsDetail) {
              ...
              // 优先取该会话已知智能体，避免被覆盖
              const finalAgent =
                sessionAgentMemory[targetSessId] || activeId || 'none';
  ...
  React.useEffect(() => {
    const handleChanged = async (e: any) => {
      ...
      if (
        e.detail?.sessionId &&
        currentSessionId &&
        e.detail.sessionId !== currentSessionId
      ) {
        return;
      }
      if (id) {
  ...
  const currentLabel =
    agentList.find((e) => e.id === currentAgent)?.label || '智能体';
            ...
            onMouseEnter={(e) => {
              if (!open)
                e.currentTarget.style.background =
                  'var(--dsw-alias-interactive-bg-hover, rgba(0,0,0,0.05))';
            }}
            onMouseLeave={(e) => {
