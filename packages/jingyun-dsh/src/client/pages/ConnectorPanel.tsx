...
const FeishuLogo = () => (
  <svg width="36" height="36" viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg">
    <path d="M24 4C12.95 4 4 12.95 4 24C4 35.05 12.95 44 24 44C35.05 44 44 35.05 44 24C44 12.95 35.05 4 24 4ZM28.59 29.83L24 34.42L19.41 29.83C15.82 26.24 15.82 20.41 19.41 16.82C23 13.23 28.83 13.23 32.42 16.82C36.01 20.41 36.01 26.24 32.42 29.83H28.59ZM24 24C25.1 24 26 23.1 26 22C26 20.9 25.1 20 24 20C22.9 20 22 20.9 22 22C22 23.1 22.9 24 24 24Z" fill="#3370FF"/>
  </svg>
)
...
// 对话气泡发送图标
const SendIcon = () => (
  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" style={{ opacity: 0.6, flexShrink: 0 }}>
...
const DingtalkLogo = () => (
  <svg width="36" height="36" viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg">
    <path d="M24 4C12.95 4 4 12.95 4 24C4 35.05 12.95 44 24 44C35.05 44 44 35.05 44 24C44 12.95 35.05 4 24 4ZM31.95 19.48C30.93 23.63 26.35 29.02 20.17 31.79C19.34 32.16 18.52 31.42 18.78 30.56C19.78 27.27 21.6 23.27 22.58 19.72C22.75 19.11 22.37 18.54 21.75 18.66C18.64 19.26 14.88 20.69 11.95 22.25C11.39 22.55 10.74 21.99 11.08 21.46C13.88 17.15 19.26 11.59 25.96 9.43C26.78 9.16 27.53 9.94 27.23 10.76C26.06 13.98 24.32 18.12 23.41 21.43C23.24 22.04 23.63 22.61 24.25 22.49C27.35 21.89 31.11 20.46 34.04 18.9C34.6 18.6 35.25 19.16 34.91 19.69C34.25 20.7 33.15 22.25 31.95 19.48Z" fill="#007FFF" fillOpacity="0.6"/>
  </svg>
)
...
const WechatWorkLogo = () => (
  <svg width="36" height="36" viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg">
    <path d="M24 4C12.95 4 4 12.95 4 24C4 35.05 12.95 44 24 44C35.05 44 44 35.05 44 24C44 12.95 35.05 4 24 4ZM28.5 28.5C28.5 29.33 27.83 30 27 30H21C20.17 30 19.5 29.33 19.5 28.5V25.5H16.5C15.67 25.5 15 24.83 15 24C15 23.17 15.67 22.5 16.5 22.5H19.5V19.5C19.5 18.67 20.17 18 21 18H27C27.83 18 28.5 18.67 28.5 19.5V22.5H31.5C32.33 22.5 33 23.17 33 24C33 24.83 32.33 25.5 31.5 25.5H28.5V28.5Z" fill="#1875F0" fillOpacity="0.6"/>
  </svg>
)
...
export const ConnectorPanel = () => {
  ...
  const handleNewChatWithPrompt = (promptText: string) => {
    try {
      ...
      setTimeout(() => {
        const textarea: any = document.querySelector('textarea[placeholder="给智能体发消息"]') ||
      ...
      console.error('[ConnectorPanel] Failed to route new chat with prompt:', e)
    }
  }
  ...
  const handleDisconnect = async () => {
    if (!confirm('确定要断开飞书的连接吗？断开后相关的飞书智能体能力将无法使用。')) return
    setLoading(true)
    try {
      const res = await fetch('/api/jingyun/connectors/lark/auth-logout', { method: 'POST' })
      if (res.ok) {
      ...
      {loading ? (
        ...
        <div style={{
          ...
          <div 
            ...
            onMouseEnter={e => {
              e.currentTarget.style.borderColor = 'var(--dsw-alias-label-tertiary, #cbd5e1)'
...
const ConnectorDetailModal = ({ channel, title, userName, appId, onClose, onDisconnect, onTryIt, onSendPrompt }: DetailModalProps) => {
  return (
    <div style={{
      position: 'fixed',
      top: 0,
      ...
      <div style={{
        ...
        <div style={{ flex: 1, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '16px', paddingRight: '2px' }}>
          ...
          <div style={{
            ...
            <div style={{ marginBottom: '8px', fontWeight: 600, color: 'var(--dsw-alias-label-primary, #0f172a)' }}>功能说明：</div>
            支持通过飞书账号授权，使 AI 具备读取及编辑飞书文档、发送即时聊天消息、配置任务、安排日历等多场景协同能力。
            <div style={{ borderTop: '1px dashed var(--dsw-alias-border, #e2e8f0)', marginTop: '12px', paddingTop: '12px', fontSize: '11.5px', display: 'flex', flexDirection: 'column', gap: '4px' }}>
              <div>授权账号：<strong style={{ color: 'var(--dsw-alias-label-primary, #0f172a)' }}>{userName}</strong></div>
              <div>应用 AppID：<span style={{ fontFamily: 'monospace' }}>{appId}</span></div>
            </div>
          </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
              {PROMPT_SUGGESTIONS.map((item, idx) => (
                ...
                >
                  <span style={{ flex: 1, textOverflow: 'ellipsis', overflow: 'hidden', whiteSpace: 'nowrap' }} title={item.text}>
                    {item.text}
                  </span>
                  <SendIcon />
                </div>
        ...
        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '10px', paddingTop: '16px', borderTop: '1px solid var(--dsw-alias-border, #e2e8f0)', marginTop: '20px' }}>
          <button
...
// 扫码授权弹窗组件
const ConnectorAuthModal = ({ channel, title, onClose, onSuccess }: AuthModalProps) => {
  const [loading, setLoading] = useState(true)
  const [authData, setAuthData] = useState<{ verification_url: string; device_code: string; mode?: string } | null>(null)
  ...
  useEffect(() => {
    startAuth()
    return () => {
      if (intervalRef.current && typeof intervalRef.current.close === 'function') {
        ...
        {isSuccess ? (
          ...
          <>
            ...
            <p style={{
              margin: '0 0 20px 0',
              fontSize: '12px',
              color: authMode === 'init' ? '#d97706' : 'var(--dsw-alias-label-tertiary, #64748b)',
              textAlign: 'center',
              lineHeight: '1.4',
              ...
              {authMode === 'init' ? (
                ...
                <>
                  <strong style={{ display: 'block', color: '#16a34a', marginBottom: '6px', fontSize: '13px' }}>第二步：应用授权</strong>
                  请继续使用手机飞书扫描下方二维码进行登录，<br/>或者在电脑浏览器里打开配置网址完成授权。
                </>
              )}
            </p>
            ...
            <div style={{
              ...
              <button
                ...
                onMouseEnter={e => { e.currentTarget.style.background = 'var(--dsw-alias-bg-card, #f8fafc)' }}
