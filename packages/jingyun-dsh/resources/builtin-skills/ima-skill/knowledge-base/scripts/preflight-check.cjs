...
function validateImageSignature(
  ...
  if (!detectedType) {
    fail({
      ...
      reason:
        'Image signature is invalid or unsupported. PNG, JPEG, and WebP files are supported.',
    });
  }
  ...
  if (
    extension &&
    extensionMapping &&
    (extensionMapping.media_type !== 9 ||
      extensionMapping.content_type !== detectedType)
  ) {
    fail({
      ...base,
      file_size: data.length,
...
if (!args.file) {
  console.error(
    'Usage: node preflight-check.cjs --file <path> [--content-type <mime>]'
  );
  process.exit(2);
}
...
const nonFileByCt = inputContentType
  ? NON_FILE_CT[inputContentType]
  : undefined;
if (nonFileByCt) {
...
const ctMediaType = inputContentType
  ? CONTENT_TYPE_MAP[inputContentType]
  : undefined;
const extMapping = ext ? EXT_MAP[ext] : undefined;

if (ctMediaType !== null && ctMediaType !== undefined) {
  ...
  if (extMapping) {
    ...
    fail({
      ...base,
      reason: `Unrecognized file extension .${ext}. This file type is not supported.`,
    });
  } else {
    fail({
      ...base,
      reason:
        'File has no extension and no --content-type provided. Cannot determine file type.',
    });
  }
}
